...
export default function Form(props) {
  ...
  const submitForm = async () => {
    const res = await fetch("/api/SpotifySearchTracks", {
  ...
  return (
    <div className="container">
      <Head>
        <title>SpotifySearchTracks form</title>
      </Head>
      <main>
        <h1>{props.title}</h1>
        <form onSubmit={event => { event.preventDefault(); submitForm() }}>
            <label htmlFor="query">query</label><input id="query" type="text" onChange={updateFormVariables(setFormVariables, ["query"], (value) => value)} />
            <input type="submit" />
          </form>
        {needsLoginService ? (
          <button
...
export async function getServerSideProps(context) {
  ...
  return {
    props: {
      title: "SpotifySearchTracks form",
      siteId: siteId
    }
  }
}
