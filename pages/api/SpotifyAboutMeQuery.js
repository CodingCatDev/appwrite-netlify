...
exports.handler = async (req, res) => {
  ...
  const { errors, data } = await NetlifyGraph.fetchSpotifyAboutMeQuery({  }, {accessToken: accessToken}); 
...
/**
async function fetchSpotifyAboutMeQuery(params) {
  const {} = params || {};
  const resp = await fetch(`/api/SpotifyAboutMeQuery?`, {
    method: "GET"
  });
